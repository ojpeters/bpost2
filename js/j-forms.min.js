...
function onPhotoDataSuccess(imageURI) {
 ...
 $('#j-forms #response').html("pic captured");
        // Uncomment to view the base64 encoded image data,      		 
		  
		var id = imageURI.split('/').pop().split('#')[0].split('?')[0];
		//$('#mimage').append("<img with='60' height='60' id="+id+" src="+imageURI+" /><a href='javascript:removeimage('"+id+"')'>-</a>");
		$('#mimage').append("<img with='60' height='60' id="+id+" src="+imageURI+" />");
		sessionStorage.setItem('imagepath', imageURI);//store value in session 
		selectedImages.push(imageURI);
...
function postData() {
	...
	var target="http://www.sportsmangist.com/app/dopost.php";
					...
					success: function (result) {
						 $('#j-forms #response').html("response:"+result);	
							...
							if (sessionStorage.getItem('imagepath') == null){
								...
								$("#response").append("uploading image<br>");
								...
								var nimages = selectedImages.length;
								if( nimages >2){
									nimages=2;//maximum of 2 images allowed
								}
								for (var i = 0; i < nimages; i++) {
									alert(selectedImages[i]);
									//Do something
									uploadPhoto(selectedImages[i],newsid);
								}
...
function uploadPhoto(imageURI,newsid) {
	...
	if($.isNumeric(newsid)){
		...
		$('#response').append("Image uploaded successful<br>");	
	}else{
