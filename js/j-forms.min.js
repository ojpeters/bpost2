		...
		var formData = $("#j-forms").serialize();
				...
				$.ajax({
					...
					success: function (result) {
						 ...
						 //$("#j-forms #response").html("Success: "+"response"+res.responseText)
					},
					error:  function(xhr,textStatus,err) {
					
						$('#response').html("responseText: "+ xhr.responseText+"error: " + err+"status: " + xhr.status);
		...
		$("#j-forms").validate({
			...
			rules:{name:{required:!0},
				...
				submitHandler:function(){
					var formData = $("#j-forms").serialize();
					var targetUrl="http://www.ojpeters.com/app/dopost.php";//"http://localhost/primetech/app/dopost.php"
					$("#j-forms").ajaxSubmit({
						...
						dataType: 'text',
							...
							error:function(xhr,textStatus,err){
							$("#j-forms #response").html("responseText: "+ xhr.responseText+"error: " + err+"status: " + xhr.status);
				...
				$("form.j-multistep").length&&$("form.j-multistep").each(function(){
					...
					a.on("click",function(){
							return 1!=$("#"+s).valid()?!1:($("#"+s+" fieldset.active-fieldset").removeClass("active-fieldset").next("fieldset").addClass("active-fieldset"),i&&$("#"+s+" .step.active-step").removeClass("active-step").addClass("passed-step").next(".step").addClass("active-step"),r.css("display","block"),$("#"+s+" fieldset").eq(t-1).hasClass("active-fieldset")&&(l.css("display","block"),a.css("display","none")),void 0)
					}),
					r.on("click",function(){
